    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    ...
                    {['critical', 'high', 'medium', 'low'].map((level) => (
                        <button
                            ...
                            className={`bg-[#141414] rounded-xl border p-4 text-left transition-colors ${urgencyFilter === level ? 'border-[#10a37f] bg-[#1a1a1a]' : 'border-[#2a2a2a] hover:border-[#333]'
                                }`}
                        >
            ...
            <div className="flex gap-3">
                ...
                {urgencyFilter && (
                    <button onClick={() => setUrgencyFilter('')} className="px-4 py-2.5 bg-[#141414] border border-[#2a2a2a] rounded-lg text-sm text-[#a1a1a1] hover:text-white hover:bg-[#1a1a1a] transition-colors">
                        Clear
                    </button>
                )}
            </div>
                                ...
                                filteredEntities.map((entity) => (
                                    <tr key={entity.id} className="hover:bg-[#1a1a1a] transition-colors">
                                        <td className="px-4 py-3">
                                            <p className="text-sm text-white font-medium">{entity.identifier}</p>
                                            {entity.metadata?.hostname && <p className="text-xs text-[#666]">{entity.metadata.hostname}</p>}
                                        </td>
                                        <td className="px-4 py-3">
                                            <span className="text-xs text-[#a1a1a1] font-mono">{entity.type || entity.entity_type}</span>
                                        </td>
                                        <td className="px-4 py-3">
